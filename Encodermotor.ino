...
#include <geometry_msgs/Twist.h>
 #include "CytronMotorDriver.h" // Initialize motor driver 
...
CytronMD motor1(PWM_DIR, pwm1, dir1); //  = PWM pin, 7 = DIR pin
CytronMD motor2(PWM_DIR, pwm2, dir2); // 3 = PWM pin, 7 = DIR pin
...
const int PWM_MAX = 190; // about 0.172 m/s
...
double leftangle = 160;
double rightangle = 160;
...
void calc_pwm_values(const geometry_msgs::Twist& cmdVel) {
  ...
  if (cmdVel.angular.z != 0.0) {
    ...
    if (cmdVel.linear.x > 0.0) {//right traj
      rightangle = -PWM_TURN;
      leftangle = PWM_TURN;
    } 
    else{//left traj
...
void set_pwm_values() {
  ...
  if ((pwmLeftReq < 0)&& (pwmRightReq<0)) { // forward
    digitalWrite(dir2, HIGH);
    digitalWrite(dir1, HIGH);
    motor1.setSpeed(-pwmRightReq);
    motor2.setSpeed(-pwmLeftReq);

  }
  else if ((pwmLeftReq > 0)&& (pwmRightReq>0)) { // BACKWARD
    digitalWrite(dir2, HIGH);
    digitalWrite(dir1, HIGH);
    motor1.setSpeed(-pwmRightReq);
    motor2.setSpeed(-pwmLeftReq);
...
else if((rightangle<0)&&(pwmRightReq>0)){
    digitalWrite(dir2, HIGH); 
    digitalWrite(dir1, HIGH); 
    motor1.setSpeed(-pwmRightReq);
    motor2.setSpeed(-pwmRightReq+30);
}  
else if((leftangle>0)&&(pwmLeftReq>0)){
    digitalWrite(dir2, HIGH); 
    digitalWrite(dir1, HIGH); 
    motor1.setSpeed(-pwmLeftReq);
    motor2.setSpeed(-pwmLeftReq+30);
} 
  // Set the direction of the motors
  else if (pwmLeftReq > 0) { // Left wheel forward
...
*/
 /* else if (pwmLeftReq == 0 && pwmLeftOut == 0 ) { // Left wheel stop
  ...
  else if (pwmRightReq > 0) { // Right wheel forward
    digitalWrite(dir1, LOW);
    digitalWrite(dir2, HIGH);    
    motor1.setSpeed(pwmRightReq);
